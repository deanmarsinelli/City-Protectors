...
function DecisionTreeBrain:_BuildDecisionTree()
	self._root = IsObjectCloseNode:Create({_teapot = self._teapot, _testObjId = g_GameObjectMgr:GetPlayer():GetObjectId()});
...
function IsObjectCloseNode:Decide()
	if (self._testObjId) then
		local object = g_GameObjectMgr:GetObjectById(self._testObjId);
		if (object) then
			local objectPos = Vec3:Create(object:GetPos());
			local teapotPos = Vec3:Create(self._teapot:GetPos());
			local diff = objectPos - teapotPos;
